...
bool RobotIo::initialize(int domain_id, const std::string &robot_namespace,
                         const std::string &cyclonedds_xml_path) {
  const std::string cyclonedds_xml = loadCycloneConfig(cyclonedds_xml_path);
  igris_c_sdk::ChannelFactory::Instance()->Init(domain_id, robot_namespace,
                                                cyclonedds_xml);
  ...
  lowstate_sub_ =
      std::make_unique<igris_c_sdk::Subscriber<igris_c::msg::dds::LowState>>(
  ...
  lowcmd_pub_ =
      std::make_unique<igris_c_sdk::Publisher<igris_c::msg::dds::LowCmd>>(
          "rt/lowcmd", igris_c_sdk::QosProfile::SensorData());
...
bool RobotIo::snapshotState(igris_c::msg::dds::LowState &state,
...
bool RobotIo::publish(const InferenceCommand &command) {
  ...
  low_cmd.header(BuildHeader(++publish_seq_, "igris_c_gmt_public"));
