...
from general_motion_retargeting.utils.params import DEFAULT_HAND_POSE, DEFAULT_MIMIC_OBS
...
def get_interpolated_obs(state_machine):
    ...
    interp_obs = (
        ...
        + (state_machine.interpolation_target_obs - state_machine.interpolation_start_obs)
        * progress
    )
...
class StateMachine:
    ...
    def update(self, controller_data):
        ...
        right_index_trig_current = controller_data.get("RightController", {}).get(
            "index_trig", False
        )
        ...
        left_axis_click_just_pressed = (
            left_axis_click_current and not self.left_axis_click_was_pressed
        )
    ...
    def get_hand_pose(self, robot_name):
        ...
        if not use_pinch:
            ...
            if (
                ...
                left_closed = (
                    left_fully_open * ratio_closed + (1 - ratio_closed) * left_fully_closed
                )
                right_open = right_fully_open * ratio_open + (1 - ratio_open) * right_fully_closed
                right_closed = (
                    right_fully_open * ratio_closed + (1 - ratio_closed) * right_fully_closed
                )
    ...
    def apply_smooth(self, mimic_obs):
        ...
        obs_array = (
            np.array(mimic_obs) if not isinstance(mimic_obs, np.ndarray) else mimic_obs.copy()
        )
    ...
    def _emergency_stop(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["pkill", "-f", "sim2real.sh"], capture_output=True, text=True, timeout=5
            )
            ...
            if result2.returncode == 0:
                print(
                    "[EMERGENCY STOP] Successfully killed server_low_level_g1_real_future.py process"
                )
            else:
                print(
                    f"[EMERGENCY STOP] pkill for server script returned code {result2.returncode}"
                )
...
class XRobotTeleopToRobot:
    def __init__(self, args):
        ...
        self.enable_motion_save = bool(self.args.save_pkl_enabled) and (
            self.args.save_pkl_dir is not None
        )
        self.motion_save_dir = (
            pathlib.Path(self.args.save_pkl_dir) if self.enable_motion_save else None
        )
    ...
    def update_motion_recording_toggle(self, controller_data):
        ...
        if (
            right_key_changed
            and (now - self.motion_toggle_last_time) >= self.motion_toggle_debounce_s
        ):
    ...
    def save_motion_chunk(self, force=False):
        ...
        if self.motion_keybody_ids:
            ...
            for q in qpos_arr:
                ...
                keybody_pos_samples.append(
                    self.motion_mj_data_save.xpos[self.motion_keybody_ids].copy()
                )
                keybody_rot_samples.append(
                    self.motion_mj_data_save.xquat[self.motion_keybody_ids].copy()
                )
        ...
        save_path = (
            self.motion_save_dir / f"{self.motion_save_prefix}_{self.motion_chunk_idx:04d}.pkl"
        )
    ...
    def _handle_enter_teleop(self, previous_state, current_retarget_obs):
        ...
        if previous_state == "idle":
            if current_retarget_obs is not None:
                ...
                start_interpolation(
                    self.state_machine, default_obs, current_retarget_obs[: self.mimic_obs_dim]
                )
            ...
            if current_retarget_obs is not None and self.state_machine.last_mimic_obs is not None:
                last_obs = (
                    self.state_machine.last_mimic_obs[: self.mimic_obs_dim]
                    if len(self.state_machine.last_mimic_obs) > self.mimic_obs_dim
                    else self.state_machine.last_mimic_obs
                )
                start_interpolation(
                    self.state_machine, last_obs, current_retarget_obs[: self.mimic_obs_dim]
                )
    ...
    def _get_teleop_mimic_obs(self, current_retarget_obs):
        ...
        if current_retarget_obs is not None:
            obs = (
                current_retarget_obs[: self.mimic_obs_dim]
                if len(current_retarget_obs) > self.mimic_obs_dim
                else current_retarget_obs
            )
    ...
    def handle_exit_sequence(self, viewer):
        ...
        if self.state_machine.current_mimic_obs is not None:
            default_obs = self.default_mimic_obs
            current_obs = (
                self.state_machine.current_mimic_obs[: self.mimic_obs_dim]
                if len(self.state_machine.current_mimic_obs) > self.mimic_obs_dim
    ...
    def initialize_all_systems(self):
        ...
        if self.state_machine.enable_smooth:
            print(
                f"- Smooth filtering: ENABLED (window size: {self.state_machine.smooth_window_size} frames)"
            )
        else:
        ...
        if self.fps_monitor.enable_detailed_stats:
            print(
                f"- FPS measurement: ENABLED (detailed stats every {self.fps_monitor.detailed_print_interval} steps)"
            )
        else:
            print(
                f"- FPS measurement: Quick stats only (every {self.fps_monitor.quick_print_interval} steps)"
            )
        if self.enable_motion_save:
            print(f"- Retargeted motion save: ENABLED (toggle with Right key_one: ON/OFF)")
